  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        {/* Payment rule box */}
        <div className="md:col-span-3 bg-gradient-to-r from-slate-50 to-gray-50 rounded-xl border border-gray-200 p-4 shadow-sm flex items-center gap-4">
          <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0">
            <DollarSign className="w-5 h-5 text-blue-600" />
          </div>
          <div>
            <p className="text-sm font-semibold text-gray-800">Payment Split Rule</p>
            <p className="text-xs text-gray-500 mt-0.5">
              For deliverable-based activities: <span className="font-bold text-blue-600">60%</span> upon submission of draft,{' '}
              <span className="font-bold text-emerald-600">40%</span> upon acceptance of final version.
              Item D1 (supervision) is paid evenly monthly over the Works Contract period.
            </p>
          </div>
        </div>
      </div>
                  ...
                  <tbody>
                    {items.map((item, idx) => (
                      <tr key={item.itemNo} className={`border-b border-gray-50 hover:bg-gray-50/50 transition-colors ${idx % 2 === 0 ? '' : 'bg-gray-25'}`}>
                        ...
                        <td className="text-right px-4 py-2.5 text-xs font-semibold text-blue-600">{fmtCurrency(item.draftMilestone)}</td>
                        <td className="text-right px-4 py-2.5 text-xs font-semibold text-emerald-600">{fmtCurrency(item.finalMilestone)}</td>
                      </tr>
                    ))}
                    {/* Group subtotal */}
                    <tr className={`${meta.bg} border-t-2 ${meta.border}`}>
                      ...
                      <td className="text-right px-4 py-2.5 text-xs font-bold text-blue-600">
                        {fmtCurrency(items.reduce((sum, i) => sum + (i.draftMilestone || 0), 0))}
                      </td>
                      <td className="text-right px-4 py-2.5 text-xs font-bold text-emerald-600">
                        {fmtCurrency(items.reduce((sum, i) => sum + (i.finalMilestone || 0), 0))}
                      </td>
                    </tr>
                  </tbody>
                </table>
      ...
      <div className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-xl p-5 shadow-lg text-white">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right space-y-1">
            <div>
              <span className="text-xs text-blue-300 mr-2">60% Draft Total:</span>
              <span className="text-sm font-bold text-blue-300">
                {fmtCurrency(schedule.activities.filter(a => !a.isGroup).reduce((sum, i) => sum + (i.draftMilestone || 0), 0))}
              </span>
            </div>
            <div>
              <span className="text-xs text-emerald-300 mr-2">40% Final Total:</span>
              <span className="text-sm font-bold text-emerald-300">
                {fmtCurrency(schedule.activities.filter(a => !a.isGroup).reduce((sum, i) => sum + (i.finalMilestone || 0), 0))}
              </span>
            </div>
            <div>
              <span className="text-xs text-amber-300 mr-2">Monthly (D1):</span>
              <span className="text-sm font-bold text-amber-300">{fmtCurrency(5166000)}</span>
            </div>
          </div>
        </div>
      </div>
